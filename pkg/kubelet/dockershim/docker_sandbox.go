...
func (ds *dockerService) RunPodSandbox(ctx context.Context,
	...
	// Step 1: 拉取sandbox的image, 默认为"k8s.gcr.io/pause:3.1"
	...
	// 接下来是network相关操作, 如果是host network, 可以直接退出了
	// Do not invoke network plugins if in hostNetwork mode.
	if config.GetLinux().GetSecurityContext().GetNamespaceOptions().GetNetwork() == runtimeapi.NamespaceMode_NODE {
		return resp, nil
	}

	// Step 5: 配置sandbox的network(CNI)
...
func (ds *dockerService) StopPodSandbox(ctx context.Context, r *runtimeapi.StopPodSandboxRequest) (*runtimeapi.StopPodSandboxResponse, error) {
	...
	// 停止network相关资源. <network>.TearDownPod
...
func (ds *dockerService) RemovePodSandbox(ctx context.Context,
	r *runtimeapi.RemovePodSandboxRequest) (*runtimeapi.RemovePodSandboxResponse, error) {
	podSandboxID := r.PodSandboxId
	var errs []error
	...
	// 列出所有的容器, 包含label: "io.kubernetes.sandbox.id": sandboxID
	containers, err := ds.client.ListContainers(opts)
	if err != nil {
		errs = append(errs, err)
	}

	// 移除所有sandbox中 running container
...
func (ds *dockerService) PodSandboxStatus(ctx context.Context,
	req *runtimeapi.PodSandboxStatusRequest) (*runtimeapi.PodSandboxStatusResponse, error) {
